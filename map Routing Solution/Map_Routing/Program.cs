    ...
    public class Program
    {
        ...
        static async Task Main(string[] args)
        {
            try
            {
                ...
                try
                {
                    graphLines = await File.ReadAllLinesAsync(@".\TestCases\Large Cases\[3] Large Cases\Input\SFMap.txt");
                    queriesLines = await File.ReadAllLinesAsync(@".\TestCases\Large Cases\[3] Large Cases\Input\SFQueries.txt");
                }
                catch (Exception ex)
                {
